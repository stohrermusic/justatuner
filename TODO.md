...
- **Low-quality-input warning on both tabs** — the drone MIC line and a new tuner readout under the motor pilot go amber below 32 kHz ("Bluetooth-grade: expect lag and coarse pitch"). Bluetooth mic input is a hands-free telephony channel (8/16/24 kHz, speech codec + DSP, 100–300 ms lag): pitch *range* is fine even at 16 kHz, signal quality and timing are not. The point of the rate fallback is not "tune with AirPods", it's "don't die when macOS makes AirPods the default input".
