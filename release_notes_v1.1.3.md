...
- **Tuner > Settings… opens again.** Two helpers the settings dialog relies on (the input-device list and the hover tooltips) were dropped in the extraction from Stohrer Sax Shop Companion, so the dialog raised an error instead of opening in v1.1.1 and v1.1.2. Both are back.
...
- **A MIC lamp on both tabs.** The tuner gets a small indicator light to the right of the VU meter, and the drone tab's status panel gets the same lamp. **Green**: good input. **Amber** with a short note: "low quality input" when the mic opened at a Bluetooth-grade rate (a hands-free headset mic is a telephony channel with a speech codec, noise gating, and 100–300 ms of lag, so it reads pitch but won't feel like a strobe tuner), or "no signal" when the mic is open but has delivered nothing but digital silence for a few seconds, which is what a denied macOS microphone permission or a muted input looks like. **Dark**: no mic input. Previously a failed mic was silently discarded, so a dead drone tab gave no clue.
- **Help > Test Audio Latency…** plays three short chirps through the speakers, listens for them on the mic, and reports the real round-trip delay — the lag you feel on the strobe wheels — alongside the audio driver's own (much smaller, buffering-only) estimate. The driver's figure can't see a Bluetooth link or a headset's DSP; this test can. Use speakers, not headphones, and expect about three seconds of test tone. Some laptop microphones cancel their own speaker output and hide the tone; the test says so rather than guessing.
