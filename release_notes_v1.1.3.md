...
- **Both tabs show mic status.** The drone tab's status panel gains a MIC line and the tuner gets a small MIC readout under the motor pilot. Normally they just say the input rate ("44.1 kHz"). If the mic opened at a Bluetooth-grade rate (under 32 kHz) they turn **amber** and say so: a hands-free headset mic is a telephony channel with a speech codec, noise gating, and 100–300 ms of lag, so it will read pitch but won't feel like a strobe tuner. Red means the mic couldn't be opened, with the reason. Previously a failed mic was silently discarded, so a dead drone tab gave no clue.
